...
class TestPixSpline(unittest.TestCase):
    ...
    def test_cen2bounds(self):
        ...
        dxedge = N.diff(xedge)
        self.assertTrue(N.all(dxedge > 0))
        self.assertTrue(N.all(dxedge == dxedge[0]))
    ...
    def test_pixspline(self):
        ...
        self.assertTrue(N.max(N.abs(dy2)) < 1e-12)

        y5 = y.reshape((nx/5,5)).mean(1)
        dy5 = sp.resample(edges[0::5]) - y5
        self.assertTrue(N.max(N.abs(dy5)) < 1e-12)
